...
export default function TransferLeads() {
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      alert(`Successfully transferred ${selectedLeads.length} leads`);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-xl">Loading transfer leads...</div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Transfer Leads</h1>

      <div className="bg-white p-6 rounded-lg shadow mb-6">
        <h2 className="text-xl font-semibold mb-4">
          Select Target Sales Person
        </h2>
        <select
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">Select Leads to Transfer</h2>
          <button
            onClick={handleTransfer}
            disabled={selectedLeads.length === 0 || !targetSalesPerson}
            className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed"
          >
            Transfer {selectedLeads.length} Leads
          </button>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {leads.map((lead) => {
                const currentSalesPerson = salesTeam.find(
